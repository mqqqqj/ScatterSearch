...
int main(int argc, char **argv)
{
    ...
    {
        std::cout << argv[0]
                  << " data_file query_file nsg_path search_L_list search_K intra_num inter_num gt_path dataset_name"
                  << std::endl;
    ...
    int K = atoi(argv[5]);
    int intra_num = atoi(argv[6]);
    int inter_num = atoi(argv[7]);
    std::vector<std::vector<unsigned>> groundtruth;
    load_groundtruth(argv[8], groundtruth);
    std::string dataset_name = argv[9];
    if (query_num > 10000)
        query_num = 10000;
    query_num = 1000;
    ...
    for (int L : L_list)
    {
        std::vector<boost::dynamic_bitset<>> flags_list(inter_num);
        for (unsigned i = 0; i < inter_num; i++)
        {
            flags_list[i] = boost::dynamic_bitset<>(points_num, 0);
        }
        std::vector<std::vector<unsigned>> res(query_num);
        std::vector<float> latency_list(query_num); // 单位：毫秒
        auto s = std::chrono::high_resolution_clock::now();
#pragma omp parallel for num_threads(inter_num) schedule(static)
        for (unsigned i = 0; i < query_num; i++)
        {
            int thread_id = omp_get_thread_num();
            boost::dynamic_bitset<> &local_flags = flags_list[thread_id];
        ...
        engine.ub_ratio = 0;
        TestResult tr{L, qps, avg_latency, avg_recall, recalls[recalls.size() * 0.05], recalls[recalls.size() * 0.01], (float)engine.dist_comps / query_num, (float)engine.hop_count / (query_num * intra_num)};
